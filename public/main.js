...
/***/ function(module, exports, __webpack_require__) {
	...
	var _jsbuddhabrot = __webpack_require__(299);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _drawBuddhabrot = __webpack_require__(300);
	
	var _drawBuddhabrot2 = _interopRequireDefault(_drawBuddhabrot);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	exports.default = _drawBuddhabrot2.default;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _BuddhabrotGenerator = __webpack_require__(302);
	
	var _BuddhabrotGenerator2 = _interopRequireDefault(_BuddhabrotGenerator);
	
	var _DensityPlot = __webpack_require__(305);
	...
	var getDrawFunc = function getDrawFunc(drawer, fractalGenerator, plot, config) {
	  ...
	  return function draw() {
	    ...
	    var pointsToPlot = fractalGenerator.next().value;
	
	    for (var i = 0; i < pointsToPlot.length; i++) {
	      // TODO Why are 2 & -1.5 being added here?
	      var x = Math.floor(scale * (pointsToPlot[i].real + 2)); // TODO what exactly is scale doing here?  And how did I pick a scale of 200?  It could be that's just what the example I saw used.
	      var y = Math.floor(-scale * (pointsToPlot[i].imaginary - 1.5)); // TODO find a way to change scale dynamically from UI - the density plot & image dimensions/rendering should be decoupled.
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _MandelbrotSequence = __webpack_require__(303);
	
	var _MandelbrotSequence2 = _interopRequireDefault(_MandelbrotSequence);
	
	var _ComplexNumber = __webpack_require__(304);
	...
	var executeIteration = function executeIteration(complexToTest, sequenceEscapeThreshold, sequenceBound) {
		...
		var i = 0,
		    current = sequenceGenerator.next().value; // first sequence member is always Complex.zero
	...
	var getRandomNumberFromPlaneRegion = function getRandomNumberFromPlaneRegion() {
		return (0, _ComplexNumber2.default)(Math.random() * 3 - 2, Math.random() * 3 - 1.5);
	};
	...
	function _callee(c) {
		...
		return regeneratorRuntime.wrap(function _callee$(_context) {
			while (1) {
				switch (_context.prev = _context.next) {
					case 0:
						current = _ComplexNumber2.default.zero;
						_context.next = 3;
						return current;
					...
					case 3:
						...
						current = _ComplexNumber2.default.add(_ComplexNumber2.default.pow(current, 2), c); // z(n + 1) = z(n)^2 + c
						_context.next = 7;
						return current;
...
/***/ function(module, exports) {
	...
	var ComplexNumber = function ComplexNumber(real, imaginary) {
		return {
			real: real,
			imaginary: imaginary,
			get magnitude() {
	...
	ComplexNumber.add = function (first, second) {
		return ComplexNumber(first.real + second.real, first.imaginary + second.imaginary);
	};
	
	ComplexNumber.multiply = function (first, second) {
	...
	ComplexNumber.pow = function (complex, pow) {
		var current = complex;
		for (var i = 0; i < pow; i++) {
